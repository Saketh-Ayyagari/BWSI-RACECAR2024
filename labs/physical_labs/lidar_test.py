...
def update_point_map():
   ...
   for a in range(360):
      ...
      if distance > 0:
         ...
         # plotting each point
         rc_utils.draw_circle(image, (CAR_POSITION[0] - rc_utils.clamp(floor(vertical_distance*0.5), -CAR_POSITION[0]+1, CAR_POSITION[0]), 
                                      CAR_POSITION[1] + rc_utils.clamp(floor(horizontal_distance*0.5), -CAR_POSITION[1], CAR_POSITION[1]-1)), 
                                      (0, 255, 0), radius=2)
